...
def test_upload_document_invalid_extensions_shows_error() :
     # 1. 로그인
    ...
    try:
        ...
        if "지원하지 않는" in alert_text:
            print("테스트 통과: 지원하지 않는 확장자 오류 메시지 확인됨")
            alert.accept()
        else:
            alert.accept()
        ...
        pytest.fail("지원하지 않는 확장자 업로드 시 오류 메시지가 표시되지 않음")
