...
def test_003_document_upload_valid_extension_pdf():
...
def test_003_document_upload_valid_extension_docx():
    ...
    # 3. 문서 파일 경로 지정
    print("업로드 할 DOCX 파일 경로")
    relative_path = '../../src/resources/asserts/files/test_normal.docx'
    file_path = get_file_path(relative_path)
    file_name = os.path.basename(file_path) 
    
    # 4. 문서 파일 업로드
    print("DOCX 파일 업로드 시도 중")
    file_input_element = wait.until(
    EC.presence_of_element_located((By.CSS_SELECTOR, 'input[type="file"]'))
    )
    ...
    # 테스트 성공 여부 확인
    uploaded_image = driver.find_element(By.XPATH, f"//span[text()='{file_name}']")
    assert uploaded_image.is_displayed(), "업로드된 DOCX 파일 카드가 화면에 나타나지 않았습니다."
    print("== 파일 업로드 완료 ==")

# [DOC_MDL_TC_004] 미지원 확장자 문서 업로드 시 에러 메시지가 표시되는지 확인

def test_004_document_invalid_extensions_shows_error_zip() :
    ...
    # 4. 문서 파일 업로드
    print("미지원 확장자[ZIP] 문서 파일 업로드 시도 중")
    file_input_element = wait.until(
    EC.presence_of_element_located((By.CSS_SELECTOR, 'input[type="file"]'))
    )
    #file_input_element = driver.find_element(By.CSS_SELECTOR, 'input[type="file"]')
    ...
    try :
        ...
        print("[ZIP] 문서 파일 카드 생성 확인 중")
        wait.until(
            EC.presence_of_element_located((By.XPATH, f'//span[text()="{file_name}"]'))
        )
        pytest.fail("지원하지 않는 확장자[ZIP] 문서 파일 업로드 시 파일 카드가 표시되면 안 됩니다.")
    ...
    try:
        ...
        assert "지원하지 않는" in alert_text, \
                f"지원하지 않는 확장자[ZIP]에 대한 오류 메시지가 표시되어야 합니다. (실제 메시지: {alert_text})"

        alert.accept()

    except TimeoutException:
        pytest.fail(
            "지원하지 않는 확장자[ZIP]의 문서 파일 업로드 시 오류 메시지가 표시되어야 합니다."
        )

def test_004_document_invalid_extensions_shows_error_exe() :
    ...
    # 4. 문서 파일 업로드
    print("미지원 확장자[EXE] 문서 파일 업로드 시도 중")
    file_input_element = wait.until(
    EC.presence_of_element_located((By.CSS_SELECTOR, 'input[type="file"]'))
    )
    #file_input_element = driver.find_element(By.CSS_SELECTOR, 'input[type="file"]')
    ...
    try :
        ...
        print("[EXE] 문서 파일 카드 생성 확인 중")
        wait.until(
            EC.presence_of_element_located((By.XPATH, f'//span[text()="{file_name}"]'))
        )
        pytest.fail("지원하지 않는 확장자[EXE] 문서 파일 업로드 시 파일 카드가 표시되면 안 됩니다.")
    ...
    try:
        ...
        # "지원하지 않는" 문구가 포함되어 있는지 확인
        assert "지원하지 않는" in alert_text, \
                f"지원하지 않는 확장자[EXE]에 대한 오류 메시지가 표시되어야 합니다. (실제 메시지: {alert_text})"

        alert.accept()

    except TimeoutException:
        pytest.fail(
            "지원하지 않는 확장자[EXE]의 문서 파일 업로드 시 오류 메시지가 표시되어야 합니다."
        )

def test_004_document_invalid_extensions_shows_error_mp4() :
    ...
    relative_path = '../../src/resources/asserts/files/test_fail.mp4'
    file_path = get_file_path(relative_path)
    file_name = os.path.basename(file_path) 
    
    # 4. 영상 파일 업로드
    print("미지원 확장자[MP4] 파일 업로드 시도 중")
    file_input_element = wait.until(
    EC.presence_of_element_located((By.CSS_SELECTOR, 'input[type="file"]'))
    )
    #file_input_element = driver.find_element(By.CSS_SELECTOR, 'input[type="file"]')
    ...
    # 영상 파일 첨부 성공 여부 확인 (파일 카드 생성이 나타나면 안 됨)
    
    try :
        
        print("[MP4] 영상 파일 카드 생성 확인 중")
        wait.until(
            EC.presence_of_element_located((By.XPATH, f'//span[text()="{file_name}"]'))
        )
        pytest.fail("지원하지 않는 확장자[MP4] 영상 파일 업로드 시 파일 카드가 표시되면 안 됩니다.")
    ...
    try:
        ...
        # "지원하지 않는" 문구가 포함되어 있는지 확인
        assert "지원하지 않는" in alert_text, \
                f"지원하지 않는 확장자[MP4]에 대한 오류 메시지가 표시되어야 합니다. (실제 메시지: {alert_text})"
...
def test_005_document_upload_exceeds_max_size_shows_error_50mb():
...
def test_005_document_upload_exceeds_max_size_shows_error_50_1mb():
...
def test_005_document_upload_exceeds_max_size_shows_error_51mb():
...
def test_005_document_upload_exceeds_max_size_shows_error_60mb():
...
def test_009_document_upload_corrupted_header_shows_error():
    ...
    try :
        ...
        pytest.fail("손상된 문서 파일 업로드 시 파일 카드가 표시되면 안 됩니다.")
