...
def test_document_upload_exceeds_max_size_shows_error_pdf():
...
def test_document_upload_exceeds_max_size_shows_error_doc():
    ...
    # 3. 문서 파일 경로 지정
    print("업로드 할 DOC 문서 파일 경로")
    relative_path = '../../src/resources/asserts/files/test_doc_60mb.doc'
    file_path = get_file_path(relative_path)
    file_name = os.path.basename(file_path) 
    
     # 4. 문서 파일 업로드
    print("DOC 문서 파일 업로드 시도 중")
    file_input_element = wait.until(
        EC.presence_of_element_located((By.CSS_SELECTOR, 'input[type="file"]'))
    )
    file_input_element.send_keys(file_path)
